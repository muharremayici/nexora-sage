...
OUTPUT_JSON = RAW_DIR / "genome_sqlite_first_access_validation.json"
OUTPUT_MD = REPORTS_DIR / "genome_sqlite_first_access_validation.md"

ARTIFACT_ID = "genome"
ARTIFACT_FILE = sqlite_first_artifact_file(ARTIFACT_ID)
HOT_GENOME_CONSUMERS = sqlite_first_hot_consumers(ARTIFACT_ID)
ALLOWED_DIRECT_GENOME_PATH_REFERENCES = sqlite_first_allowed_references(ARTIFACT_ID)
...
def _is_genome_json_constant(node: ast.AST) -> bool:
    if isinstance(node, ast.Constant) and str(node.value) == ARTIFACT_FILE:
        return True
    if isinstance(node, ast.JoinedStr):
        return any(_is_genome_json_constant(value) for value in node.values)
    return False


def _contains_genome_json(node: ast.AST) -> bool:
    return any(_is_genome_json_constant(child) for child in ast.walk(node))


def _direct_genome_consumption_calls(path: Path) -> list[dict[str, Any]]:
    ...
    for node in ast.walk(tree):
        ...
        if name not in {"load_json_file", "load_json_strict", "_load_json", "json_load", "load_json"}:
            continue
        if not _contains_genome_json(node):
            continue
...
def validate() -> dict[str, Any]:
    ...
    missing_pipeline_hot_consumers = [
        ...
        if file not in HOT_GENOME_CONSUMERS
    ]
    ...
    for path in python_files:
        if "__pycache__" in path.parts:
            continue
        direct_calls.extend(_direct_genome_consumption_calls(path))
        references.extend(_path_reference_lines(path))

    hot_direct = [
        item
        for item in direct_calls
        if item.get("file") in HOT_GENOME_CONSUMERS and item.get("file") != CENTRAL_ADAPTER
    ]
    undeclared_references = [
        item
        for item in references
        if item.get("file") not in ALLOWED_DIRECT_GENOME_PATH_REFERENCES
        and item.get("file") not in HOT_GENOME_CONSUMERS
    ]
    hot_imports: dict[str, dict[str, Any]] = {}
    for rel_path in sorted(HOT_GENOME_CONSUMERS):
        path = ROOT / rel_path
        if not path.exists():
        ...
        hot_imports[rel_path] = {
            "passed": has_central_api if touches_genome else True,
            "touches_genome": touches_genome,
            "uses_central_api": has_central_api,
            "matched_central_apis": matched_apis,
            "reason": "central API required" if touches_genome else "not applicable",
        }

    checks = [
        {
            "name": "central_genome_io_is_sqlite_first",
            "passed": bool(SQLITE_FIRST_MARKER) and SQLITE_FIRST_MARKER in (ROOT / CENTRAL_ADAPTER).read_text(encoding="utf-8", errors="replace"),
            "details": "Default Genome reads must go through ArtifactStore SQLite-first path.",
        },
        {
            "name": "hot_genome_consumers_use_central_genome_api",
            "passed": all(item.get("passed") for item in hot_imports.values()),
            "details": hot_imports,
        },
        {
            "name": "pipeline_declared_genome_modules_are_hot_consumers",
            "passed": not missing_pipeline_hot_consumers,
            "details": missing_pipeline_hot_consumers or "all pipeline-declared genome modules are policy hot consumers",
        },
        {
            "name": "genome_path_references_are_policy_declared",
            "passed": not undeclared_references,
            "details": undeclared_references or "all genome.json path references are policy-declared",
        },
        {
            "name": "hot_genome_consumers_do_not_direct_load_genome_json",
            "passed": not hot_direct,
            "details": hot_direct or "no hot direct genome.json load calls",
        },
    ]
    status = "PASS" if all(check["passed"] for check in checks) else "FAIL"
    payload = {
        "meta": {"kind": "genome_sqlite_first_access_validation", "version": "v1"},
        "summary": {
            "status": status,
            "checks": len(checks),
            "passed": sum(1 for check in checks if check["passed"]),
            "direct_genome_load_calls": len(direct_calls),
            "hot_direct_genome_load_calls": len(hot_direct),
            "genome_path_references": len(references),
            "undeclared_reference_candidates": len(undeclared_references),
            "pipeline_declared_genome_modules": len(pipeline_declared_modules),
            "missing_pipeline_hot_consumers": len(missing_pipeline_hot_consumers),
        },
        "checks": checks,
        "direct_genome_load_calls": direct_calls,
        "undeclared_reference_candidates": undeclared_references[:100],
        "pipeline_declared_genome_modules": pipeline_declared_modules,
        "missing_pipeline_hot_consumers": missing_pipeline_hot_consumers,
        "allowed_reference_policy": ALLOWED_DIRECT_GENOME_PATH_REFERENCES,
    }
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Genome SQLite-First Access Validation",
        "",
        f"- status: `{summary.get('status')}`",
        f"- direct genome load calls: `{summary.get('direct_genome_load_calls')}`",
        f"- hot direct genome load calls: `{summary.get('hot_direct_genome_load_calls')}`",
        f"- genome path references: `{summary.get('genome_path_references')}`",
        f"- undeclared reference candidates: `{summary.get('undeclared_reference_candidates')}`",
        f"- pipeline-declared genome modules: `{summary.get('pipeline_declared_genome_modules')}`",
